...
- [x] **Anthony Bourdain's Libya, Ethiopia and Nicaragua episodes were excluded (`no_home_nonstop`)** — done 2026-08-26, and grew into all 35 countries Ivan confirmed Bourdain visited (Libya, Ethiopia, Madagascar, Mozambique, Tanzania, DRC, Namibia, Liberia, Nigeria, Senegal, Saint Vincent and the Grenadines, Nicaragua, Paraguay, Uruguay, Bhutan, Iran, Armenia, Georgia, Iraq, Oman, Palestine, Croatia, Romania, Australia, New Zealand, Cambodia, Indonesia, Laos, Malaysia, Myanmar, the Philippines, Sri Lanka, Thailand, Uzbekistan, Vietnam). Not the one-stop-connection model this item originally proposed — Ivan asked for the simpler fix instead: add the documented trips anyway with the carrier recorded as **"Airline Unknown"** rather than modeled or invented. `chef_trips.py` gained an `assumed_flight` per-episode opt-in for exactly this (real distance, real destination, no real carrier), used on 32 of the 35 (Nigeria and Senegal already had a real nonstop; Palestine was already covered inside the Jerusalem episode, filed under Israel). See [[chef_travelers]] for the mechanism and [[bourdain_trips]] for the two research corrections it turned up along the way (the "Congo" episode is Kinshasa/DRC, not Brazzaville; "Caribbean Island Hopping" resolves to Saint Vincent and the Grenadines after all).
...
- [ ] **Classify each trip by tourist type**, using one of two established typologies (pick one — don't build both):
  ...
  Needs research before it's buildable: this is a per-TRIP categorical label (unlike the continuous per-traveler preference vector above), so it needs a rule or model mapping each trip's existing fields — `accommodation_type`, `transportation_type`, `duration_days`, destination entropy/repeat-destination behavior (see [[traveler_entropy]]), whether the destination is a resort/beach vs. an off-the-beaten-path one — onto one of the categories. Cohen's four-way split (structured around how much of the trip is organized vs. improvised, familiar vs. novel) is the simpler starting point; Yiannakis & Gibson's finer roles would need more signals than the dataset currently carries for most trips (Kaggle-sourced ones especially — no group size, no itinerary detail, no traveler-stated motivation).
