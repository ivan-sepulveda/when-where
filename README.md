...
- [ ] Add a third 100% stacked bar to the traveler page (`/rec-sys/travelers/:travelerId`) for **destination continent**, alongside the existing "Airlines flown" and "Domestic vs international" bars. `components/StackedShareBar.tsx` already takes any `ShareBreakdown`, so this is a new function in `lib/travelerCharts.ts` plus one more `<StackedShareBar>` — no chart work. **What's actually missing is the data**: nothing in `data/reference/` maps a country to a continent. `tourist_cities.json` carries `iso2`/`iso3` but no region; the only `region` fields in the repo are UNESCO's own regional grouping (`unesco_by_country.json`), which is not M49 and shouldn't be reused for this.
- [ ] **Decide the continent scheme first — the two lists in the brief don't agree.** The continent bar was specced as *North America, South America, Oceania, Europe, Asia* (Americas split, Africa absent), but M49's top tier is *Africa, Americas, Asia, Europe, Oceania* (Americas merged, Africa present) with "Northern America" and "South America" living one level down as subregions. Both can't be the same tier of the same hierarchy. Note that dropping Africa would silently discard real trips — the dataset currently has 6 (South Africa 4, Morocco 1, Egypt 1) across 44 destination countries. Either use M49's five regions for the continent bar and let Northern/South America show up in the subregion bar, or use a colloquial seven-continent scheme and accept that it isn't M49.
- [ ] Add a fourth 100% stacked bar for **UN M49 subregion** (Northern Africa, Eastern Africa, …, Australia and New Zealand, Melanesia, Micronesia, Polynesia — 22 in total). Source: the UN Statistics Division's M49 standard, https://unstats.un.org/unsd/methodology/m49/. The site is built for browsing rather than bulk download, so this probably means a `fetch_m49_regions.py` that scrapes the "Full view" overview into `data/reference/m49_regions.json`, keyed by ISO-alpha3 (which `tourist_cities.json` already carries as `iso3`, so it joins cleanly) with `region`, `subregion` and the optional `intermediate_region`. M49's rule that "each country or area is shown in one region only" means the join is 1:1 and totals can't double-count.
