...
- [ ] **Anthony Bourdain's Libya, Ethiopia and Nicaragua episodes are currently excluded (`no_home_nonstop`)** — he definitely went (Parts Unknown S1.E6 Libya/Tripoli 2013-05-19, Parts Unknown S6.E6 Ethiopia/Addis Ababa 2015-10-25, No Reservations S7.E3 Nicaragua/Managua 2011-03-14 — see `build_bourdain_trips.py`), but there's no nonstop from JFK/LGA/EWR to any of TIP/BEN, ADD or MGA in `airline_routes_enhanced.csv`, almost certainly because there never was one, not because the data's wrong. Explore how to add these three back in — likely means modeling a real one-stop connection (which hub, which carrier) rather than the nonstop-substitution trick used elsewhere (e.g. Rick Steves' regional-episode fallbacks, David Chang's Marrakech->Casablanca — see [[chef_travelers]]), since a same-country substitute airport doesn't exist for any of these three the way it does there. Needs research per episode, not a blanket rule change to `chef_trips.py`'s flights-only filter.
